...
impl Chunk{
    ...
    pub fn GreedyMesh(&mut self, adj: &[Option<*const Chunk>; 4], blockRegistry: &BlockRegistry){
        ...
        let getBlock = |x: &mut [i32; 3], dim: usize| -> Block {
            let temp = x[dim];
            x[dim] = (x[dim] + 1) / 2;
            let block = self.Blocks[To1Di((x[0], x[1], x[2])) as usize].clone();
            x[dim] = temp;
            block
        };
        ...
        let mut addVert = |x: i32, y: i32, z: i32, tID: i32, vID: i32, fID: i32| {
            let data = ( x  | z << 4 | y << 8 |  (tID as i32) << 16 | (vID as i32) << 24 | (fID as i32) << 26 ) as u32;
            self.Mesh.push(Vertex { Data: data } );
        };
                ...
                for j in 0..dimensions[axis2] {
                    ...
                    while i < dimensions[axis1] {
                        ...
                        if mask[n] {
                            ...
                            let fID = x[dim].rem_euclid(2) as i32 + dim as i32 * 2;
                            let mut texID = 0_i32;
                            ...
                            addVert(x[0], x[1], x[2], texID, 0, fID);                 // Top-left vertice position
                            addVert(x[0] + d1[0],x[1] + d1[1], x[2] + d1[2], texID, 1, fID);       // Top right vertice position
                            addVert(x[0] + d2[0],         x[1] + d2[1],         x[2] + d2[2], texID, 2, fID);        // Bottom left vertice position
                            addVert(x[0] + d1[0] + d2[0], x[1] + d1[1] + d2[1], x[2] + d1[2] + d2[2], texID, 3, fID);  // Bottom right vertice position
